...
export type UserForClient = {
  ...
  userRoles: {
    ...
    tenantId: string | null; // membership tenantId
  }[];
};
...
type Props = {
  ...
  /** workspace tenant (central-hive is still a tenant) */
  tenantId: string | null;
  tenantName: string | null;

  /** ✅ central admin UI mode (central roles only) */
...
export function UsersTabClient(props: Props) {
  const {
    ...
    tenantId,
    tenantName,
    isCentralAdmin: isCentralAdminProp,
    permissions = [],
    companySettings,
    brandingSettings,
  } = props;
  ...
  // ✅ central admin mode drives role dropdown
  const isCentralAdmin = !!isCentralAdminProp;
  ...
  const [selectedRowsOnPage, setSelectedRowsOnPage] = React.useState<
    UserForClient[]
  >([]);
  ...
  // ✅ FIX: role dropdown must match the current context
 const modalAssignableRoles = React.useMemo(() => assignableRoles, [assignableRoles]);


  const safeDefaultRoleId = React.useMemo(
    () => modalAssignableRoles[0]?.id ?? "",
    [modalAssignableRoles]
  );

  const isSuperadminEditing = React.useMemo(() => {
    if (!editingUser) return false;
    return editingUser.userRoles.some((ur) => {
      if (isCentralAdmin) return ur.role.key === "central_superadmin";
      return tenantId
        ? ur.role.key === "tenant_superadmin" && ur.tenantId === tenantId
        : false;
    });
  }, [editingUser, tenantId, isCentralAdmin]);
  ...
  function getPrimaryRoleName(u: UserForClient): string {
    if (isCentralAdmin) {
      const centralRole = u.userRoles.find((ur) =>
  ...
  function openEdit(user: UserForClient) {
    ...
    // ✅ FIX: pick membership based on current context
    const membership = isCentralAdmin
      ? user.userRoles.find((ur) => ur.role.key.startsWith("central_")) ||
        user.userRoles[0]
      : (tenantId
          ? user.userRoles.find((ur) => ur.tenantId === tenantId)
          : null) || user.userRoles[0];

    const roleIdFromMembership = membership?.roleId ?? null;
    setFormRoleId(roleIdFromMembership ?? safeDefaultRoleId);
  ...
  async function refetch(nextQuery: Query) {
    ...
    try {
      const payload = await fetchUsersTabAction({
        tenantId: tenantId ?? null, // workspace tenant (central-hive etc.)
        page: nextQuery.page,
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (!isSuperadminEditing && !formRoleId) {
      toast.error("No role selected. Please create roles first.");
      return;
    }
    ...
    startTransition(async () => {
      try {
        await createOrUpdateUserAction({
          ...
          // central assigns in central context
          tenantId: isCentralAdmin ? null : (tenantId ?? null),
          ...
          bulkDeletableUsers.map(async (u) => {
            try {
              await deleteUserAction({
                userId: u.id,
                tenantId: isCentralAdmin ? null : tenantId,
              });
            } catch {
              errors++;
            }
                  ...
                  startTransition(async () => {
                    try {
                      ...
                      if (msg.includes("CANNOT_DEACTIVATE_SELF"))
                        ...
                        msg.includes("FORBIDDEN_INSUFFICIENT_PERMISSIONS")
                      )
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 p-1">
        <div>
          <h2 className="text-lg font-bold tracking-tight">
            {isCentralAdmin ? "System Users" : "Team Members"}
          </h2>
      ...
      <div className="rounded-md border bg-card shadow-sm">
        <DataTable<UserForClient, any>
          ...
          onDeleteRows={() => {
            ...
            const deletable = selectedRowsOnPage.filter(
              (u) => !isProtectedUser(u)
            );
