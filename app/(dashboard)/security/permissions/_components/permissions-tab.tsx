...
import { deletePermissionAction, upsertPermissionAction } from "../permissions-actions";
...
export function PermissionsTab({
  ...
  const openEdit = React.useCallback(
    (p: PermissionWithFlag) => {
      ...
      if (p.isGlobal) {
        toast.error("Cannot edit system permission.");
        return;
      }
    ...
    (e: React.FormEvent) => {
      ...
      startTransition(async () => {
        ...
        try {
          ...
          toast.dismiss(tid);
          toast.error("Failed to save", { description: prettyError(err?.message) });
        }
      });
    },
  ...
  const doDeleteSingle = React.useCallback(
    (id: string) => {
      startTransition(async () => {
        const tid = toast.loading("Deleting permission...");
        try {
          ...
          toast.dismiss(tid);
          toast.error("Delete failed", { description: prettyError(err?.message) });
        }
      });
    },
    [router]
  ...
  const columns = React.useMemo<ColumnDef<PermissionWithFlag>[]>(
    () => [
      ...
      },
      {
        ...
        header: "Type",
        meta: {
          exportValue: (row: PermissionWithFlag) => (row.isGlobal ? "System" : "Custom"),
        },
        cell: ({ row }) =>
          row.original.isGlobal ? (
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          const deleteDisabled = !canDelete || isLocked;
              ...
              <AlertDialog>
                ...
                <AlertDialogContent>
                  <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                      This cannot be undone. If any role uses it, deletion will be blocked.
                    </AlertDialogDescription>
                  </AlertDialogHeader>
    ...
    [canDelete, canUpdate, canView, doDeleteSingle, openEdit, openView]
  );
      ...
      <div className="rounded-md border bg-card">
        <DataTable
          ...
          onDeleteRows={async (rows) => {
            ...
            // ✅ block system rows from bulk delete
            const ids = rows.filter((r: PermissionWithFlag) => !r.isGlobal).map((r: PermissionWithFlag) => r.id);
            ...
            try {
              const res = await deletePermissionAction(ids); // ✅ ONE CALL

              toast.dismiss(tid);
              if (res?.blocked) {
                toast.warning(`Deleted ${res.deleted}. Blocked ${res.blocked} (in use).`);
              } else {
              ...
              toast.dismiss(tid);
              toast.error("Bulk delete failed", { description: prettyError(err?.message) });
            }
          }}
        />
      </div>
              ...
              {form.name.trim() && (
                <div className="text-[11px] text-muted-foreground">
                  ...
                  <span className="font-mono">
                    {form.key || slugifyPermissionKey(form.name)}
                  </span>
                </div>
              )}
            </div>
